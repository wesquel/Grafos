...
class Grafo:
    ...
    def warshall(self):
        ...
        for i in range(len(self.N)):
            for j in range(len(self.N)):
                if copia_matriz[j][i] == 1:
                    for k in range(len(self.N)):
                        copia_matriz[j][k] = max(copia_matriz[j][k], copia_matriz[i][k])

        return copia_matriz

    def str_warshall(self):
        g_w = self.warshall()
        string = ' '
        for y in self.N:
            string += ' ' + str(y)
        string_final = string + '\n'
        for x in range(len(g_w)):
            string = ''
            string += str(g_w[x]).replace('[', str(self.N[x]) + ' ')

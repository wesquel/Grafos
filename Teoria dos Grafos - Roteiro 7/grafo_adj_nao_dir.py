...
class Grafo:
    ...
    def dijkstra(self, v, f, recargas, carga_atual, carga_maxima):
        vertices = self.variaveis()
        vertices[v] = [0, 1, 0, carga_atual]

        return self.caminho_dijkstra(v, f, v, recargas, vertices, carga_maxima)

    def caminho_dijkstra(self, v, f, w, recargas, vertices, carga_maxima):

        if w == f:
            return self.caminho_dijkstra_atual(v, f, vertices)[::-1]

        vizinhos = self.vizinho(w)
        check = False

        for x in vizinhos:
            calculo = vertices[w][0] + 1
            if vertices[x][0] > calculo and (calculo - vertices[w][3] != 0 or x in recargas or x == f):
                vertices[x][0] = calculo
                vertices[x][2] = w
                vertices[x][3] = vertices[w][3]

                if w in recargas and check == False:
                    soma1 = carga_maxima - (vertices[x][3] - vertices[x][0])
                    soma2 = carga_maxima - (vertices[w][3] - vertices[w][0])
                    vertices[w][3] += soma2
                    vertices[x][3] += soma1
                    check = True
        ...
        for y in self.N:
            if (vertices[y][0] < valor) and (vertices[y][1] != 1) and (
                    vertices[y][3] - vertices[y][0] > 0 or y in recargas or y == f):
                valor = vertices[y][0]
                next = y

        if next == '':
            return 'CARGA INSUFICIENTE PARA PERCORRER O CAMINHO!'

        vertices[next][1] = 1

        return self.caminho_dijkstra(v, f, next, recargas, vertices, carga_maxima)
    ...
    def variaveis(self):
        ...
        for x in range(len(self.N)):
            beta = infinito
            dicionario.update({self.N[x]: [beta, 0, 0, 0]})
        return dicionario

    def caminho_dijkstra_atual(self, v, f, vertices):
        caminho = [f]
        vfinal = vertices[f][2]
        caminho.append(vfinal)

        while vfinal != v and v != f:
            vfinal = vertices[vfinal][2]
            caminho.append(vfinal)
